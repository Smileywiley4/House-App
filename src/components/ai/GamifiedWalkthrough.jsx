...
const RESPONSES = [
  ...
  { id: "like", label: "Like it", icon: ThumbsUp, color: "text-[#106B49]", bg: "bg-propurty-green-tint border-propurty-green-tint hover:bg-propurty-green-tint" },
...
export default function GamifiedWalkthrough({ propertyAddress, assignmentId, onComplete }) {
  ...
  if (!sessionId) {
    return (
      <div className="rounded-2xl border border-[#106B49]/20 bg-white p-6 text-center">
        <Sparkles className="mx-auto mb-3 text-[#106B49]" size={28} />
        <h3 className="font-bold text-[#14192E] mb-2">Gamified walk-through</h3>
        ...
        <button
          ...
          onClick={start}
          disabled={starting}
          className="inline-flex items-center gap-2 px-5 py-3 bg-[#106B49] hover:bg-[#0C4F37] text-white font-semibold rounded-xl text-sm disabled:opacity-60"
        >
  ...
  if (done) {
    return (
      <div className="rounded-2xl border border-[#106B49]/30 bg-[#106B49]/5 p-8 text-center">
        <p className="text-[#106B49] font-bold text-lg mb-2">Tour complete</p>
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-white overflow-hidden shadow-sm">
      <div className="h-1.5 bg-slate-100">
        <div className="h-full bg-[#106B49] transition-all duration-300" style={{ width: `${progress}%` }} />
      </div>
      <div className="p-6">
        <p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">
          Question {index + 1} of {questions.length}
        </p>
        <p className="text-lg font-semibold text-[#14192E] mb-1">{q?.category_label}</p>
        ...
        {learnedHint && (
          <p className="mt-4 text-xs text-[#106B49] flex items-center gap-1">
