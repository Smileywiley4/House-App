...
export default function SharedComparison() {
  ...
  if (error) return (
    <div className="min-h-screen bg-[#F8F7F4] flex items-center justify-center">
      <div className="text-center max-w-sm">
        ...
        <h2 className="text-xl font-bold text-[#14192E] mb-2">Invalid Share Link</h2>
        <p className="text-slate-400 text-sm mb-6">This link may be expired or malformed.</p>
        <Link to={createPageUrl("Home")} className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#106B49] text-white font-bold rounded-xl text-sm">
          <HomeIcon size={15} /> Go to Propurty
        </Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#F8F7F4]">
      {/* Header */}
      <div className="bg-[#14192E] px-6 py-6 sticky top-[112px] sm:top-14 z-30">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <div className="text-[#106B49] text-xs font-bold uppercase tracking-widest mb-1">Propurty · Shared Comparison</div>
              <h1 className="text-xl font-bold text-white">
                {scores.length} Propert{scores.length !== 1 ? "ies" : "y"} Compared
              </h1>
              <p className="text-slate-400 text-xs mt-0.5">View-only · shared by a Propurty user</p>
            </div>
            <div className="flex items-center gap-3">
              <div className="flex bg-white/10 rounded-xl p-1 gap-1">
                {VIEW_MODES.map(({ id, label, icon: Icon }) => (
                  <button key={id} onClick={() => setViewMode(id)}
                    className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold transition-all ${viewMode === id ? "bg-[#106B49] text-white" : "text-slate-400 hover:text-white"}`}>
                    <Icon size={14} /><span className="hidden sm:inline">{label}</span>
                  </button>
                ))}
              </div>
              <Link to={createPageUrl("Home")}
                className="px-4 py-2 bg-[#106B49] hover:bg-[#0C4F37] text-white font-semibold rounded-xl text-xs transition">
                Try Propurty →
              </Link>
            </div>
          </div>
    ...
    <div>
      <div className="grid gap-4 mb-6" style={{ gridTemplateColumns: `180px repeat(${comparing.length}, 1fr)` }}>
        ...
        {comparing.map(p => {
          ...
          return (
            <div key={p.id} className={`bg-white rounded-2xl border p-5 text-center shadow-sm ${isWinner ? "border-[#106B49]" : "border-slate-100"}`}>
              {isWinner && <div className="flex items-center justify-center gap-1 text-[#106B49] text-xs font-bold mb-2"><Trophy size={12} /> Top Pick</div>}
              <div className="text-sm font-bold text-[#14192E] leading-tight">{p.property_address?.split(",")[0]}</div>
      ...
      <div className="space-y-2">
        {allCategories.map((cat, i) => (
          <div key={cat.id} className={`grid gap-4 rounded-xl p-4 ${i % 2 === 0 ? "bg-white border border-slate-100" : "bg-slate-50"}`}
            ...
            <div className="flex items-center"><span className="text-sm font-semibold text-[#14192E]">{cat.label}</span></div>
            {comparing.map(p => {
              ...
              return (
                <div key={p.id} className="flex flex-col items-center gap-1">
                  <span className={`text-xl font-bold ${best ? "text-[#106B49]" : "text-[#14192E]"}`}>{s.score}<span className="text-xs text-slate-400">/10</span></span>
                  <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full rounded-full bg-[#106B49] opacity-60" style={{ width: `${(s.score / 10) * 100}%` }} />
                  </div>
                </div>
              );
            })}
  ...
  return (
    <div className={`grid gap-5 ${comparing.length === 2 ? "md:grid-cols-2" : "md:grid-cols-3"}`}>
      {comparing.map(p => {
        const isWinner = p.id === winner?.id;
        return (
          <div key={p.id} className={`bg-white rounded-2xl border shadow-sm overflow-hidden ${isWinner ? "border-[#106B49]" : "border-slate-100"}`}>
            <div className={`px-6 py-5 ${isWinner ? "bg-[#106B49]/5" : ""}`}>
              {isWinner && <div className="flex items-center gap-1 text-[#106B49] text-xs font-bold mb-1"><Trophy size={12} /> Top Pick</div>}
              <h3 className="font-bold text-[#14192E]">{p.property_address?.split(",")[0]}</h3>
                  ...
                  <div key={cat.category_id} className="flex items-center justify-between py-1.5 border-b border-slate-50 last:border-0">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                        <div className="h-full rounded-full bg-[#106B49]" style={{ width: `${(cat.score / 10) * 100}%` }} />
                      </div>
            ...
            {comparing.map(p => {
              ...
              return (
                <th key={p.id} className={`px-5 py-4 text-center ${isWinner ? "bg-[#106B49]/5" : ""}`}>
                  {isWinner && <div className="flex items-center justify-center gap-1 text-[#106B49] text-[10px] font-bold mb-1"><Trophy size={10} />Top Pick</div>}
                  <div className="text-xs font-bold text-[#14192E]">{p.property_address?.split(",")[0]}</div>
              ...
              {comparing.map(p => {
                ...
                return (
                  <td key={p.id} className="px-5 py-3 text-center">
                    <span className={`text-sm font-bold ${best ? "text-[#106B49]" : "text-slate-600"}`}>{s.score}/10</span>
                  </td>
                );
              })}
            </tr>
